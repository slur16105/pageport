...
export function PaymentSuccess({ payment }: { payment: PaymentParams | null }) {
  ...
  const [message, setMessage] = useState(payment ? "주문 금액을 확인하고 시험 결제를 승인하고 있습니다." : "결제 결과 정보가 올바르지 않습니다.");
  ...
  return (
    <main className="payment-result-page">
      <section className={`payment-result-card ${status}`}>
        ...
        {status === "success" && downloadUrl && <a className="download-button" href={downloadUrl}>PDF 다운로드</a>}
        {status === "success" && <a className="result-home-link" href="/downloads/reissue">다운로드 주소 다시 받기</a>}
        <a className="result-home-link" href="/">상품 목록으로 돌아가기</a>
      </section>
    </main>
  );
}
