...
export async function sendDownloadRenewalEmail(input: { buyerEmail: string; productTitle: string; orderId: string; downloadUrl: string; expiresAt: number }) {
  const runtimeEnv = env as unknown as Record<string, string | undefined>;
  const apiKey = runtimeEnv.RESEND_API_KEY;
  if (!apiKey?.startsWith("re_")) throw new Error("Resend 이메일 발송 설정이 필요합니다.");
  const template = downloadRenewedEmail(input);
